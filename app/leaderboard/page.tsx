...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    const userEntry = leaderboardData.find((entry) => entry.id === user._id);
    setCurrentUserRank(userEntry?.rank ?? null);
    setCurrentUserData(userEntry ?? null);
  ...
  const getLevelTierBadge = (tier: string) => {
    ...
    const config =
      tierConfig[tier as keyof typeof tierConfig] || tierConfig.Beginner;
    return { ...config };
  };
          ...
          <>
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          {currentUserData.totalPointsEarned.toLocaleString()}{' '}
                          points • Level {currentUserData.level} •{' '}
                          {currentUserData.achievementCount} achievements
                        </div>
                        <div className="text-xs text-gray-600">
                          {currentUserData.totalScanned} scans •{' '}
                          {currentUserData.streakCount} day streak
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <Badge
                        className={
                          getLevelTierBadge(currentUserData.levelTier).color
                        }
                      >
                        {getLevelTierBadge(currentUserData.levelTier).icon}{' '}
                        {currentUserData.levelTier}
                      </Badge>
                      <div className="mt-1">
                        {getChangeIndicator(currentUserData.change)}
                      </div>
                    </div>
                  </div>
                </CardContent>
            ...
            {leaderboardData.length >= 3 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {leaderboardData
                  .slice(0, 3)
                  .map((userEntry: LeaderboardUser, index: number) => (
                    <Card
                      key={userEntry.id}
                      className={`bg-green-100 border-none shadow-md ${index === 0 ? 'ring-2 ring-yellow-400/50' : ''}`}
                    >
